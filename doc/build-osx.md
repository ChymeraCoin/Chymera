...
This guide describes how to build chymerad, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build chymera Core from source.
...
### 4. Clone chymera repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the chymera Core repository to a directory.
...
It is not necessary to build wallet functionality to run `chymerad` or  `chymera-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/chymera/chymera/blob/master/doc/descriptors.md), `sqlite` is required.
...
chymera Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/chymera/chymera/issues/7714).
...
You can deploy a `.dmg` containing the chymera Core application using `make deploy`.
...
## Building chymera Core

### 1. Configuration

There are many ways to configure chymera Core, here are a few common examples:
...
Run the following in your terminal to compile chymera Core:
...
## Running chymera Core

chymera Core should now be available at `./src/chymerad`.
If you compiled support for the GUI, it should be available at `./src/qt/chymera-qt`.

The first time you run `chymerad` or `chymera-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/chymera/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/chymera"

touch "/Users/${USER}/Library/Application Support/chymera/chymera.conf"

chmod 600 "/Users/${USER}/Library/Application Support/chymera/chymera.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/chymera/debug.log
```

## Other commands:

```shell
./src/chymerad -daemon      # Starts the chymera daemon.
./src/chymera-cli --help    # Outputs a list of command-line options.
./src/chymera-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/chymera-qt -server # Starts the chymera-qt server mode, allows chymera-cli control
```
