# Fuzzing chymera Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing chymera Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/chymera/chymera
$ cd chymera/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/chymera/chymera/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/chymera/chymera/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/chymera/chymera/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/chymera/chymera/tree/master/src/test/fuzz).
...
INFO: Seed: 840522292
INFO: Loaded 1 modules   (424174 inline 8-bit counters): 424174 [cx55e121ef9ab8, cx55e121f613a6),
INFO: Loaded 1 PC tables (424174 PCs): 424174 [cx55e121f613a8,cx55e1225da288),
...
The project's collection of seed corpora is found in the [`chymera-core/qa-assets`](https://github.com/chymera-core/qa-assets) repo.

To fuzz `process_message` using the [`chymera-core/qa-assets`](https://github.com/chymera-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/chymera-core/qa-assets
$ FUZZ=process_message src/test/fuzz/fuzz qa-assets/fuzz_seed_corpus/process_message/
INFO: Seed: 1346407872
INFO: Loaded 1 modules   (424174 inline 8-bit counters): 424174 [cx55d8a9004ab8, cx55d8a906c3a6),
INFO: Loaded 1 PC tables (424174 PCs): 424174 [cx55d8a906c3a8,cx55d8a96e5288),
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`chymera-core/qa-assets`](https://github.com/chymera-core/qa-assets) repo.

Every single pull request submitted against the chymera Core repo is automatically tested against all inputs in the [`chymera-core/qa-assets`](https://github.com/chymera-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make chymera Core more robust.
...
with certain assembly code from chymera Core's code. See [developer notes on sanitizers](https://github.com/chymera/chymera/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing chymera Core using afl++

## Quickstart guide

To quickly get started fuzzing chymera Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing chymera Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing chymera Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the chymera Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as chymera
Core without having to write any custom fuzzing harness. The `chymerad` server
...
```sh
$ mkdir chymera-honggfuzz-p2p/
$ cd chymera-honggfuzz-p2p/
$ git clone https://github.com/chymera/chymera
$ cd chymera/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/chymerad.cpp b/src/chymerad.cpp
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/chymerad -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
...
chymera Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/chymera-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=chymera-core).
...
because chymera is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_chymera-core/latest).
