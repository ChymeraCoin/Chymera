...
The chymera Core daemon can be configured to act as a trusted "border
router", implementing the chymera wire protocol and relay, making
...
The ZeroMQ feature in chymera Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building chymerad:
...
For instance:
    ...
    $ chymerad -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/chymerad.tx.raw \
...
These options can also be provided in chymera.conf.
...
From the perspective of chymerad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into chymerad directly. Furthermore, no information is
...
using. chymerad appends an up-counting sequence number to each
