...
//! so chymera should never create different databases with the same fileid, but
...
bool BerkeleyEnvironment::Open(bilingual_str& err)
{
    ...
    if (!LockDirectory(pathIn, ".walletlock")) {
        LogPrintf("Cannot obtain a lock on wallet directory %s. Another instance of chymera may be using it.\n", strPath);
    ...
    dbenv->set_cachesize(0, cx100000, 1); // 1 MiB should be enough for just the wallet
    dbenv->set_lg_bsize(cx10000);
...
bool BerkeleyDatabase::Verify(bilingual_str& errorStr)
{
    ...
    if (fs::exists(file_path))
    {
        ...
        if (result != 0) {
            errorStr = strprintf(_("%s corrupt. Try using the wallet tool chymera-wallet to salvage or restoring a backup."), file_path);
            return false;
        }
    }
