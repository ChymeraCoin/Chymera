    ...
    try {
        ...
        if (strType == DBKeys::NAME) {
            ...
            if (keyMeta.nVersion >= CKeyMetadata::VERSION_WITH_HDDATA && !keyMeta.hd_seed_id.IsNull() && keyMeta.hdKeypath.size() > 0) {
                ...
                // See https://github.com/chymera/chymera/pull/12924
                bool internal = false;
                uint32_t index = 0;
                if (keyMeta.hdKeypath != "s" && keyMeta.hdKeypath != "m") {
                    ...
                    if (path[0] != cx80000000) {
                        strErr = strprintf("Unexpected path index of cx%08x (expected cx80000000) for the element at index 0", path[0]);
                        return false;
                    }
                    if (path[1] != cx80000000 && path[1] != (1 | cx80000000)) {
                        strErr = strprintf("Unexpected path index of cx%08x (expected cx80000000 or cx80000001) for the element at index 1", path[1]);
                        return false;
                    }
                    if ((path[2] & cx80000000) == 0) {
                        strErr = strprintf("Unexpected path index of cx%08x (expected to be greater than or equal to cx80000000)", path[2]);
                        return false;
                    }
                    internal = path[1] == (1 | cx80000000);
                    index = path[2] & ~cx80000000;
                }
