...
const char * const chymera_CONF_FILENAME = "chymera.conf";
const char * const chymera_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/chymera/chymera/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //chymera-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", chymera_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\chymera
    // macOS: ~/Library/Application Support/chymera
    // Unix-like: ~/.chymera
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "chymera";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/chymera";
#else
    // Unix-like
    return pathRet / ".chymera";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", chymera_CONF_FILENAME);
...
void AllocateFileRange(FILE *file, unsigned int offset, unsigned int length) {
    ...
    nFileSize.u.LowPart = nEndPos & cxFFFFFFFF;
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure chymera Core copyright is not removed by accident
    if (copyright_devs.find("chymera Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The chymera Core developers";
    }
