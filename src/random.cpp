...
static constexpr uint32_t CPUID_F1_ECX_RDRAND = cx40000000;
static constexpr uint32_t CPUID_F7_EBX_RDSEED = cx00040000;
#ifdef bit_RDRND
...
static uint64_t GetRdRand() noexcept
{
    ...
    uint32_t r1 = 0, r2 = 0;
    for (int i = 0; i < 10; ++i) {
        __asm__ volatile (".byte cx0f, cxc7, cxf0; setc %1" : "=a"(r1), "=q"(ok) :: "cc"); // rdrand %eax
        if (ok) break;
    }
    for (int i = 0; i < 10; ++i) {
        __asm__ volatile (".byte cx0f, cxc7, cxf0; setc %1" : "=a"(r2), "=q"(ok) :: "cc"); // rdrand %eax
        if (ok) break;
    }
    ...
    for (int i = 0; i < 10; ++i) {
        __asm__ volatile (".byte cx48, cx0f, cxc7, cxf0; setc %1" : "=a"(r1), "=q"(ok) :: "cc"); // rdrand %rax
        if (ok) break;
    }
    return r1;
#else
...
static uint64_t GetRdSeed() noexcept
{
    ...
    uint32_t r1, r2;
    do {
        __asm__ volatile (".byte cx0f, cxc7, cxf8; setc %1" : "=a"(r1), "=q"(ok) :: "cc"); // rdseed %eax
        if (ok) break;
        __asm__ volatile ("pause");
    } while(true);
    do {
        __asm__ volatile (".byte cx0f, cxc7, cxf8; setc %1" : "=a"(r2), "=q"(ok) :: "cc"); // rdseed %eax
        if (ok) break;
        __asm__ volatile ("pause");
    } while(true);
    return (((uint64_t)r2) << 32) | r1;
    ...
    uint64_t r1;
    do {
        __asm__ volatile (".byte cx48, cx0f, cxc7, cxf8; setc %1" : "=a"(r1), "=q"(ok) :: "cc"); // rdseed %rax
        if (ok) break;
        __asm__ volatile ("pause");
    } while(true);
    return r1;
...
public:
    ...
    void AddEvent(uint32_t event_info) noexcept
    {
        ...
        // subsecond part.
        uint32_t perfcounter = (GetPerformanceCounter() & cxffffffff);
