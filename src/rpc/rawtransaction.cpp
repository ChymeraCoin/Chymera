...
{
    // Call into TxToUniv() in chymera-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in chymera-common, so we query them here and push the
                             ...
                             {RPCResult::Type::ARR, "vout", "",
                             {
                                 {RPCResult::Type::OBJ, "", "",
                                 {
                                     ...
                                     {RPCResult::Type::OBJ, "scriptPubKey", "",
                                     {
                                         ...
                                         {RPCResult::Type::STR, "address", /* optional */ true, "chymera address (only if a well-defined address exists)"},
                                         {RPCResult::Type::ARR, "addresses", /* optional */ true, "(DEPRECATED, returned only if config option -deprecatedrpc=addresses is passed) Array of chymera addresses",
                                         {
                                             {RPCResult::Type::STR, "address", "chymera address"},
                                         }},
                                     }},
                                 }},
                             }},
...
static RPCHelpMan decodepsbt()
{
    return RPCHelpMan{"decodepsbt",
                "\nReturn a JSON object representing the serialized, base64-encoded partially signed chymera transaction.\n",
                {
                            ...
                            {RPCResult::Type::OBJ, "", "",
                            {
                                ...
                                {RPCResult::Type::OBJ, "witness_utxo", /* optional */ true, "Transaction output for witness UTXOs",
                                {
                                    ...
                                    {RPCResult::Type::OBJ, "scriptPubKey", "",
                                    {
                                        ...
                                        {RPCResult::Type::STR_HEX, "hex", "The hex"},
                                        {RPCResult::Type::STR, "type", "The type, eg 'pubkeyhash'"},
                                        {RPCResult::Type::STR, "address"," chymera address if there is one"},
                                    }},
                                }},
...
static RPCHelpMan combinepsbt()
{
    return RPCHelpMan{"combinepsbt",
                "\nCombine multiple partially signed chymera transactions into one transaction.\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    uint32_t best_locktime = cxffffffff;
    for (unsigned int i = 0; i < txs.size(); ++i) {
        PartiallySignedTransaction psbtx;
        std::string error;
        if (!DecodeBase64PSBT(psbtx, txs[i].get_str(), error)) {
