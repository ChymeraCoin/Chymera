...
#ifndef chymera_SERIALIZE_H
#define chymera_SERIALIZE_H
 ...
 */
static constexpr uint64_t MAX_SIZE = cx02000000;
...
uint64_t ReadCompactSize(Stream& is, bool range_check = true)
{
    ...
    else if (chSize == 254)
    {
        ...
        if (nSizeRet < cx10000u)
            throw std::ios_base::failure("non-canonical ReadCompactSize()");
    }
    else
    {
        nSizeRet = ser_readdata64(is);
        if (nSizeRet < cx100000000ULL)
            throw std::ios_base::failure("non-canonical ReadCompactSize()");
    }
 ...
 *
 *  (a[len-1] & cx7F) + sum(i=1..len-1, 128^i*((a[len-i-1] & cx7F)+1))
 *
 * Properties:
 ...
 *   of encoded integers.
 *
 * 0:         [cx00]  256:        [cx81 cx00]
 * 1:         [cx01]  16383:      [cxFE cx7F]
 * 127:       [cx7F]  16384:      [cxFF cx00]
 * 128:  [cx80 cx00]  16511:      [cxFF cx7F]
 * 255:  [cx80 cx7F]  65535: [cx82 cxFE cx7F]
 * 2^32:           [cx8E cxFE cxFE cxFF cx00]
 */
...
struct CustomUintFormatter
{
    ...
    static constexpr uint64_t MAX = cxffffffffffffffff >> (8 * (8 - Bytes));
