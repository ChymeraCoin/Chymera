...
BOOST_AUTO_TEST_CASE(outbound_slow_chain_eviction)
{
    ...
    auto connman = std::make_unique<CConnman>(cx1337, cx1337, *m_node.addrman);
    auto peerLogic = PeerManager::make(chainparams, *connman, *m_node.addrman, nullptr,
                                       *m_node.scheduler, *m_node.chainman, *m_node.mempool, false);

    // Mock an outbound peer
    CAddress addr1(ip(cxa0b0c001), NODE_NONE);
...
{
    const CChainParams& chainparams = Params();
    auto connman = std::make_unique<CConnmanTest>(cx1337, cx1337, *m_node.addrman);
    auto peerLogic = PeerManager::make(chainparams, *connman, *m_node.addrman, nullptr,
                                       *m_node.scheduler, *m_node.chainman, *m_node.mempool, false);
...
BOOST_AUTO_TEST_CASE(peer_discouragement)
{
    ...
    const std::array<CAddress, 3> addr{CAddress{ip(cxa0b0c001), NODE_NONE},
                                       CAddress{ip(cxa0b0c002), NODE_NONE},
                                       CAddress{tor_service, NODE_NONE}};

    const CNetAddr other_addr{ip(cxa0b0ff01)}; // Not any of addr[].
...
BOOST_AUTO_TEST_CASE(DoS_bantime)
{
    ...
    CAddress addr(ip(cxa0b0c001), NODE_NONE);
