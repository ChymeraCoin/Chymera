// Copyright (c) 2012-2019 The chymera Core developers
...
BOOST_AUTO_TEST_CASE(arena_tests)
{
    ...
    void *synth_base = reinterpret_cast<void*>(cx08000000);
    ...
    for (int x=0; x<5000; ++x) {
        ...
        s >>= 1;
        if (lsb)
            s ^= cxf00f00f0; // LFSR period cxf7ffffe0
    }
    for (void *ptr: addr)
        b.free(ptr);
    addr.clear();
    ...
    void* AllocateLocked(size_t len, bool *lockingSuccess) override
    {
        *lockingSuccess = false;
        if (count > 0) {
            ...
            return reinterpret_cast<void*>(uint64_t{static_cast<uint64_t>(cx08000000) + (count << 24)}); // Fake address, do not actually use this memory
        }
        return nullptr;
    }
...
BOOST_AUTO_TEST_CASE(lockedpool_tests_live)
{
    ...
    *((uint32_t*)a0) = cx1234;
    BOOST_CHECK(*((uint32_t*)a0) == cx1234);
