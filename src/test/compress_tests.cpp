...
// amounts 1 .. 10000
#define NUM_MULTIPLES_1cmr 10000

// amounts 50 .. 21000000
#define NUM_MULTIPLES_50cmr 420000
...
{
    BOOST_CHECK(TestPair(            0,       cx0));
    BOOST_CHECK(TestPair(            1,       cx1));
    BOOST_CHECK(TestPair(         CENT,       cx7));
    BOOST_CHECK(TestPair(         COIN,       cx9));
    BOOST_CHECK(TestPair(      50*COIN,      cx32));
    BOOST_CHECK(TestPair(21000000*COIN, cx1406f40));
    ...
    for (uint64_t i = 1; i <= NUM_MULTIPLES_1cmr; i++)
        BOOST_CHECK(TestEncode(i * COIN));

    for (uint64_t i = 1; i <= NUM_MULTIPLES_50cmr; i++)
...
BOOST_AUTO_TEST_CASE(compress_script_to_uncompressed_pubkey_id)
{
    ...
    BOOST_CHECK_EQUAL(out[0], cx04 | (script[65] & cx01)); // least significant bit (lsb) of last char of pubkey is mapped into out[0]
}
