...
BOOST_AUTO_TEST_CASE(signet_parse_tests)
{
    ...
    std::vector<uint8_t> witness_commitment_section_141{cxaa, cx21, cxa9, cxed};
    for (int i = 0; i < 32; ++i) {
        witness_commitment_section_141.push_back(cxff);
    }
    ...
    std::vector<uint8_t> witness_commitment_section_325{cxec, cxc7, cxda, cxa2};
    ...
    witness_commitment_section_325.push_back(cx01);
    witness_commitment_section_325.push_back(cx51);
    cb.vout.at(0).scriptPubKey = CScript{} << OP_RETURN << witness_commitment_section_141 << witness_commitment_section_325;
    block.vtx.at(0) = MakeTransactionRef(cb);
    BOOST_CHECK(!SignetTxs::Create(block, challenge));
    BOOST_CHECK(!CheckSignetBlockSolution(block, signet_params->GetConsensus()));
    ...
    // has data, valid
    witness_commitment_section_325.push_back(cx00);
    cb.vout.at(0).scriptPubKey = CScript{} << OP_RETURN << witness_commitment_section_141 << witness_commitment_section_325;
    block.vtx.at(0) = MakeTransactionRef(cb);
    BOOST_CHECK(SignetTxs::Create(block, challenge));
    BOOST_CHECK(CheckSignetBlockSolution(block, signet_params->GetConsensus()));
