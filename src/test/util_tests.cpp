...
static const unsigned char ParseHex_expected[65] = {
    cx04, cx67, cx8a, cxfd, cxb0, cxfe, cx55, cx48, cx27, cx19, cx67, cxf1, cxa6, cx71, cx30, cxb7,
    cx10, cx5c, cxd6, cxa8, cx28, cxe0, cx39, cx09, cxa6, cx79, cx62, cxe0, cxea, cx1f, cx61, cxde,
    cxb6, cx49, cxf6, cxbc, cx3f, cx4c, cxef, cx38, cxc4, cxf3, cx55, cx04, cxe5, cx1e, cxc1, cx12,
    cxde, cx5c, cx38, cx4d, cxf7, cxba, cx0b, cx8d, cx57, cx8a, cx4c, cx70, cx2b, cx6b, cxf1, cx1d,
    cx5f
};
BOOST_AUTO_TEST_CASE(util_ParseHex)
{
    ...
    BOOST_CHECK(result.size() == 4 && result[0] == cx12 && result[1] == cx34 && result[2] == cx56 && result[3] == cx78);

    // Leading space must be supported (used in BerkeleyEnvironment::Salvage)
    result = ParseHex(" 89 34 56 78");
    BOOST_CHECK(result.size() == 4 && result[0] == cx89 && result[1] == cx34 && result[2] == cx56 && result[3] == cx78);

    // Stop parsing at invalid value
    result = ParseHex("1234 invalid 1234");
    BOOST_CHECK(result.size() == 2 && result[0] == cx12 && result[1] == cx34);
}
...
BOOST_AUTO_TEST_CASE(util_ArgParsing)
{
    ...
    TestParse("cx", false, 0);
    TestParse("5x", true, 5);
    TestParse("cx5", false, 0);
...
BOOST_FIXTURE_TEST_CASE(util_ArgsMerge, ArgsMergeTestingSetup)
{
    ...
    //
    //   ARGS_MERGE_TEST_OUT=results.txt ./test_chymera --run_test=util_tests/util_ArgsMerge
    //
...
BOOST_FIXTURE_TEST_CASE(util_ChainMerge, ChainMergeTestingSetup)
{
    ...
    // If check below fails, should manually dump the results with:
    //
    //   CHAIN_MERGE_TEST_OUT=results.txt ./test_chymera --run_test=util_tests/util_ChainMerge
    //
    // And verify diff against previous results to make sure the changes are expected.
    //
    // Results file is formatted like:
...
BOOST_AUTO_TEST_CASE(util_IsHex)
{
    ...
    BOOST_CHECK(!IsHex("0cxx00"));
    BOOST_CHECK(!IsHex("cx0000"));
}

BOOST_AUTO_TEST_CASE(util_IsHexNumber)
{
    BOOST_CHECK(IsHexNumber("cx0"));
    BOOST_CHECK(IsHexNumber("0"));
    BOOST_CHECK(IsHexNumber("cx10"));
    BOOST_CHECK(IsHexNumber("10"));
    BOOST_CHECK(IsHexNumber("cxff"));
    BOOST_CHECK(IsHexNumber("ff"));
    BOOST_CHECK(IsHexNumber("cxFfa"));
    BOOST_CHECK(IsHexNumber("Ffa"));
    BOOST_CHECK(IsHexNumber("cx00112233445566778899aabbccddeeffAABBCCDDEEFF"));
    BOOST_CHECK(IsHexNumber("00112233445566778899aabbccddeeffAABBCCDDEEFF"));

    BOOST_CHECK(!IsHexNumber(""));   // empty string not allowed
    BOOST_CHECK(!IsHexNumber("cx")); // empty string after prefix not allowed
    BOOST_CHECK(!IsHexNumber("cx0 ")); // no spaces at end,
    BOOST_CHECK(!IsHexNumber(" cx0")); // or beginning,
    BOOST_CHECK(!IsHexNumber("cx 0")); // or middle,
    BOOST_CHECK(!IsHexNumber(" "));    // etc.
    BOOST_CHECK(!IsHexNumber("cx0ga")); // invalid character
    BOOST_CHECK(!IsHexNumber("x0"));    // broken prefix
    BOOST_CHECK(!IsHexNumber("cxcx00")); // two prefixes not allowed
 ...
 * Remove this test before time.ctime(cxffffffff) == 'Sun Feb  7 07:28:15 2106'
 */
BOOST_AUTO_TEST_CASE(gettime)
{
    BOOST_CHECK((GetTime() & ~cxFFFFFFFFLL) == 0);
}
...
BOOST_AUTO_TEST_CASE(test_ParseInt32)
{
    ...
    BOOST_CHECK(!ParseInt32("cx1", &n)); // no hex
...
BOOST_AUTO_TEST_CASE(test_ParseInt64)
{
    ...
    BOOST_CHECK(!ParseInt64("cx1", &n)); // no hex
...
BOOST_AUTO_TEST_CASE(test_ParseUInt8)
{
    ...
    BOOST_CHECK(!ParseUInt8("cx1", &n)); // no hex
...
BOOST_AUTO_TEST_CASE(test_ParseUInt16)
{
    ...
    BOOST_CHECK(!ParseUInt16("cx1", &n)); // no hex
...
BOOST_AUTO_TEST_CASE(test_ParseUInt32)
{
    ...
    BOOST_CHECK(!ParseUInt32("cx1", &n)); // no hex
...
BOOST_AUTO_TEST_CASE(test_ParseUInt64)
{
    ...
    BOOST_CHECK(!ParseUInt64("cx1", &n)); // no hex
...
BOOST_AUTO_TEST_CASE(test_ParseDouble)
{
    ...
    BOOST_CHECK(!ParseDouble("cx1", &n)); // no hex
...
BOOST_AUTO_TEST_CASE(test_Capitalize)
{
    ...
    BOOST_CHECK_EQUAL(Capitalize("chymera"), "chymera");
...
BOOST_AUTO_TEST_CASE(message_sign)
{
    const std::array<unsigned char, 32> privkey_bytes = {
        ...
        cxD9, cx7F, cx51, cx08, cxF1, cx1C, cxDA, cx6E,
        cxEE, cxBA, cxAA, cx42, cx0F, cxEF, cx07, cx26,
        cxB1, cxF8, cx98, cx06, cx0B, cx98, cx48, cx9F,
        cxA3, cx09, cx84, cx63, cxC0, cx03, cx28, cx66
    };
