...
TEST(Crc32CTest, Crc32c) {
  ...
  EXPECT_EQ(static_cast<uint32_t>(cx8a9136aa),
            crc32c::Crc32c(buf, sizeof(buf)));

  std::memset(buf, cxff, sizeof(buf));
  EXPECT_EQ(static_cast<uint32_t>(cx62a8ab43),
            crc32c::Crc32c(buf, sizeof(buf)));

  for (size_t i = 0; i < 32; ++i)
    buf[i] = static_cast<uint8_t>(i);
  EXPECT_EQ(static_cast<uint32_t>(cx46dd794e),
            crc32c::Crc32c(buf, sizeof(buf)));

  for (size_t i = 0; i < 32; ++i)
    buf[i] = static_cast<uint8_t>(31 - i);
  EXPECT_EQ(static_cast<uint32_t>(cx113fdb5c),
            crc32c::Crc32c(buf, sizeof(buf)));
  ...
  EXPECT_EQ(static_cast<uint32_t>(cxd9963a56),
...
TEST(CRC32CTest, Crc32cStdString) {
  ...
  for (size_t i = 0; i < 32; ++i)
    buf[i] = static_cast<char>(cx00);
  EXPECT_EQ(static_cast<uint32_t>(cx8a9136aa), crc32c::Crc32c(buf));

  for (size_t i = 0; i < 32; ++i)
    buf[i] = '\xff';
  EXPECT_EQ(static_cast<uint32_t>(cx62a8ab43), crc32c::Crc32c(buf));

  for (size_t i = 0; i < 32; ++i)
    buf[i] = static_cast<char>(i);
  EXPECT_EQ(static_cast<uint32_t>(cx46dd794e), crc32c::Crc32c(buf));

  for (size_t i = 0; i < 32; ++i)
    buf[i] = static_cast<char>(31 - i);
  EXPECT_EQ(static_cast<uint32_t>(cx113fdb5c), crc32c::Crc32c(buf));
}
...
TEST(CRC32CTest, Crc32cStdStringView) {
  ...
  std::memset(buf, 0, sizeof(buf));
  EXPECT_EQ(static_cast<uint32_t>(cx8a9136aa), crc32c::Crc32c(view));

  std::memset(buf, cxff, sizeof(buf));
  EXPECT_EQ(static_cast<uint32_t>(cx62a8ab43), crc32c::Crc32c(view));

  for (size_t i = 0; i < 32; ++i)
    buf[i] = static_cast<uint8_t>(i);
  EXPECT_EQ(static_cast<uint32_t>(cx46dd794e), crc32c::Crc32c(view));

  for (size_t i = 0; i < 32; ++i)
    buf[i] = static_cast<uint8_t>(31 - i);
  EXPECT_EQ(static_cast<uint32_t>(cx113fdb5c), crc32c::Crc32c(view));
}
