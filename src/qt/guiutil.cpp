...
#include <qt/chymeraaddressvalidator.h>
...
static const uint8_t dummydata[] = {cxeb,cx15,cx23,cx1d,cxfc,cxeb,cx60,cx92,cx58,cx86,cxb6,cx7d,cx06,cx52,cx99,cx92,cx59,cx15,cxae,cxb1,cx72,cxc0,cx66,cx47};
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a chymera address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new chymeraAddressEntryValidator(parent));
    widget->setCheckValidator(new chymeraAddressCheckValidator(parent));
}
...
bool parsechymeraURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no chymera: URI
    if(!uri.isValid() || uri.scheme() != QString("chymera"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!chymeraUnits::parse(chymeraUnits::cmr, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parsechymeraURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parsechymeraURI(uriInstance, out);
}

QString formatchymeraURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("chymera:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(chymeraUnits::format(chymeraUnits::cmr, info.amount, false, chymeraUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openchymeraConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", chymera_CONF_FILENAME));
    ...
    /* Open chymera.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "chymera.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "chymera (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("chymera (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for chymera*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "chymera.desktop";
    return GetAutostartDir() / strprintf("chymera-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a chymera.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=chymera\n";
        else
            optionFile << strprintf("Name=chymera (%s)\n", chain);
