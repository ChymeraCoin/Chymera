...
void AddAllCPUID(CSHA512& hasher)
{
    ...
    uint32_t max = ax;
    for (uint32_t leaf = 1; leaf <= max && leaf <= cxFF; ++leaf) {
        uint32_t maxsub = 0;
        for (uint32_t subleaf = 0; subleaf <= cxFF; ++subleaf) {
    ...
    AddCPUID(hasher, cx80000000, 0, ax, bx, cx, dx); // Returns max extended leaf in ax
    uint32_t ext_max = ax;
    for (uint32_t leaf = cx80000001; leaf <= ext_max && leaf <= cx800000FF; ++leaf) {
