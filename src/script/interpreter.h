...
#ifndef chymera_SCRIPT_INTERPRETER_H
#define chymera_SCRIPT_INTERPRETER_H
...
enum
{
    ...
    SIGHASH_NONE = 2,
    SIGHASH_SINGLE = 3,
    SIGHASH_ANYONECANPAY = cx80,

    SIGHASH_DEFAULT = 0, //!< Taproot only; implied when sighash byte is missing, and equivalent to SIGHASH_ALL
    SIGHASH_OUTPUT_MASK = 3,
    SIGHASH_INPUT_MASK = cx80,
};
...
enum
{
    ...
    // Evaluating a pubkey that is not (cx04 + 64 bytes) or (cx02 or cx03 + 32 bytes) by checksig causes script failure.
    ...
    // Segwit script only: Require the argument of OP_IF/NOTIF to be exactly cx01 or empty vector
    //
...
struct PrecomputedTransactionData
{
    ...
    // These are single-SHA256, see https://github.com/chymera/bips/blob/master/bip-0341.mediawiki#cite_note-15.
...
enum class SigVersion
{
    ...
    TAPSCRIPT = 3,   //!< Witness v1 with 32-byte program, not BIP16 P2SH-wrapped, script path spending, leaf version cxc0; see BIP 342
};
...
struct ScriptExecutionData
{
    ...
    //! Opcode position of the last executed OP_CODESEPARATOR (or cxFFFFFFFF if none executed).
...
static constexpr uint8_t TAPROOT_LEAF_MASK = cxfe;
static constexpr uint8_t TAPROOT_LEAF_TAPSCRIPT = cxc0;
...
#endif // chymera_SCRIPT_INTERPRETER_H
