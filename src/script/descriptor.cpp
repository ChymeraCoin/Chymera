...
{
    uint8_t c0 = c >> 35;
    c = ((c & cx7ffffffff) << 5) ^ val;
    if (c0 & 1) c ^= cxf5dee51989;
    if (c0 & 2) c ^= cxa9fdca3312;
    if (c0 & 4) c ^= cx1bab10e32d;
    if (c0 & 8) c ^= cx3706b1677a;
    if (c0 & 16) c ^= cx644d626ffd;
    return c;
}
...
public:
    ...
    bool GetPubKey(int pos, const SigningProvider& arg, CPubKey& key_out, KeyOriginInfo& final_info_out, const DescriptorCache* read_cache = nullptr, DescriptorCache* write_cache = nullptr) override
    {
        ...
        if (m_derive == DeriveType::HARDENED) final_info_out_tmp.path.push_back(((uint32_t)pos) | cx80000000L);
        ...
        if (read_cache) {
            ...
            if (m_derive == DeriveType::HARDENED) der = xprv.Derive(xprv, pos | cx80000000UL);
    ...
    bool GetPrivKey(int pos, const SigningProvider& arg, CKey& key) const override
    {
        ...
        if (m_derive == DeriveType::HARDENED) extkey.Derive(extkey, pos | cx80000000UL);
        key = extkey.key;
        return true;
    }
};
