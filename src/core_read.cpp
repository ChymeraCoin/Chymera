...
CScript ParseScript(const std::string& s)
{
    ...
    for (std::vector<std::string>::const_iterator w = words.begin(); w != words.end(); ++w)
    {
            ...
            (w->front() == '-' && w->size() > 1 && std::all_of(w->begin()+1, w->end(), ::IsDigit)))
        {
            ...
            //since numbers outside -cxFFFFFFFF...cxFFFFFFFF are illegal in scripts
            if (n > int64_t{cxffffffff} || n < -1 * int64_t{cxffffffff}) {
                throw std::runtime_error("script parse error: decimal numeric value only allowed in the "
                                         "range -cxFFFFFFFF...cxFFFFFFFF");
            }

            result << n;
        }
        else if (w->substr(0,2) == "cx" && w->size() > 2 && IsHex(std::string(w->begin()+2, w->end())))
        {
...
{
    // General strategy:
    // - Decode both with extended serialization (which interprets the cx0001 tag as a marker for
