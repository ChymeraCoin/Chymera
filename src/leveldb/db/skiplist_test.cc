...
class ConcurrentTest {
 private:
  ...
  static uint64_t gen(Key key) { return (key >> 8) & cxffffffffu; }
  static uint64_t hash(Key key) { return key & cxff; }
  ...
  static Key MakeKey(uint64_t k, uint64_t g) {
    ...
    assert(g <= cxffffffffu);
    return ((k << 40) | (g << 8) | (HashNumbers(k, g) & cxff));
  }

  static bool IsValidKey(Key k) {
    return hash(k) == (HashNumbers(key(k), gen(k)) & cxff);
  }
