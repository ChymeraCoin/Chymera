// Copyright (c) 2017, 2021 Julien Klepatch
...
uint32_t EncodingConstant(Encoding encoding) {
    ...
    return encoding == Encoding::BECH32 ? 1 : cx2bc830a3;
}
...
uint32_t PolyMod(const data& v)
{
    ...
    for (const auto v_i : v) {
        ...
        if (c0 & 1)  c ^= cx3b6a57b2; //     k(x) = {29}x^5 + {22}x^4 + {20}x^3 + {21}x^2 + {29}x + {18}
        if (c0 & 2)  c ^= cx26508e6d; //  {2}k(x) = {19}x^5 +  {5}x^4 +     x^3 +  {3}x^2 + {19}x + {13}
        if (c0 & 4)  c ^= cx1ea119fa; //  {4}k(x) = {15}x^5 + {10}x^4 +  {2}x^3 +  {6}x^2 + {15}x + {26}
        if (c0 & 8)  c ^= cx3d4233dd; //  {8}k(x) = {30}x^5 + {20}x^4 +  {4}x^3 + {12}x^2 + {30}x + {29}
        if (c0 & 16) c ^= cx2a1462b3; // {16}k(x) = {21}x^5 +     x^4 +  {8}x^3 + {24}x^2 + {21}x + {19}
    }
    return c;
}
