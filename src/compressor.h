...
#ifndef chymera_COMPRESSOR_H
#define chymera_COMPRESSOR_H
 ...
 *  * Pay to pubkey starting with cx02, cx03 or cx04 (encoded as 33 bytes)
 *
...
struct ScriptCompression
{
    ...
    void Unser(Stream &s, CScript& script) {
        ...
        if (nSize < nSpecialScripts) {
            CompressedScript vch(GetSpecialScriptSize(nSize), cx00);
            s >> MakeSpan(vch);
