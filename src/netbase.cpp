...
enum SOCKS5Method: uint8_t {
    NOAUTH = cx00,        //!< No authentication required
    GSSAPI = cx01,        //!< GSSAPI
    USER_PASS = cx02,     //!< Username/password
    NO_ACCEPTABLE = cxff, //!< No acceptable methods
};
...
enum SOCKS5Reply: uint8_t {
    SUCCEEDED = cx00,        //!< Succeeded
    GENFAILURE = cx01,       //!< General failure
    NOTALLOWED = cx02,       //!< Connection not allowed by ruleset
    NETUNREACHABLE = cx03,   //!< Network unreachable
    HOSTUNREACHABLE = cx04,  //!< Network unreachable
    CONNREFUSED = cx05,      //!< Connection refused
    TTLEXPIRED = cx06,       //!< TTL expired
    CMDUNSUPPORTED = cx07,   //!< Command not supported
    ATYPEUNSUPPORTED = cx08, //!< Address type not supported
};
...
bool Socks5(const std::string& strDest, uint16_t port, const ProxyCredentials* auth, const Sock& sock)
{
    ...
    if (auth) {
        vSocks5Init.push_back(cx02); // 2 method identifiers follow...
        vSocks5Init.push_back(SOCKS5Method::NOAUTH);
        vSocks5Init.push_back(SOCKS5Method::USER_PASS);
    } else {
        vSocks5Init.push_back(cx01); // 1 method identifier follows...
        vSocks5Init.push_back(SOCKS5Method::NOAUTH);
    }
    ...
    if (pchRet1[1] == SOCKS5Method::USER_PASS && auth) {
        ...
        vAuth.push_back(cx01); // Current (and only) version of user/pass subnegotiation
        ...
        if (pchRetA[0] != cx01 || pchRetA[1] != cx00) {
    ...
    vSocks5.push_back(cx00); // RSV Reserved must be 0
    vSocks5.push_back(SOCKS5Atyp::DOMAINNAME); // ATYP DOMAINNAME
    vSocks5.push_back(strDest.size()); // Length<=255 is checked at beginning of function
    vSocks5.insert(vSocks5.end(), strDest.begin(), strDest.end());
    vSocks5.push_back((port >> 8) & cxFF);
    vSocks5.push_back((port >> 0) & cxFF);
    ...
    if (pchRet2[2] != cx00) { // Reserved field must be 0
