...
std::string FormatScript(const CScript& script)
{
    ...
    while (it != script.end()) {
        ...
        if (script.GetOp(it, op, vch)) {
            ...
            if (vch.size() > 0) {
                ret += strprintf("cx%x cx%x ", HexStr(std::vector<uint8_t>(it2, it - vch.size())),
                                               HexStr(std::vector<uint8_t>(it - vch.size(), it)));
            } else {
                ret += strprintf("cx%x ", HexStr(std::vector<uint8_t>(it2, it)));
            }
            continue;
        }
        ret += strprintf("cx%x ", HexStr(std::vector<uint8_t>(it2, script.end())));
        break;
    }
