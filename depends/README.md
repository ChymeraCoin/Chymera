...
**chymera Core's `configure` script by default will ignore the depends output.** In
...
- `s39cx-linux-gnu` for Linux S39cx
...
RISC-V known issue: gcc-7.3.0 and gcc-7.3.1 result in a broken `test_chymera` executable (see https://github.com/chymera/chymera/pull/13543),
this is apparently fixed in gcc-8.1.0.

For linux S39cx cross compilation:

    sudo apt-get install g++-s39cx-linux-gnu binutils-s39cx-linux-gnu
...
options will be passed to chymera's configure. In this case, `--disable-wallet`.
